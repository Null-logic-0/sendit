defmodule SenditWeb.UserAuthTest do
  use SenditWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Sendit.Accounts
  alias Sendit.Accounts.Scope
  alias SenditWeb.UserAuth

  import Sendit.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SenditWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SenditWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SenditWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
