defmodule Sendit.Accounts.User do
  use Ecto.Schema
  ...
  defp validate_email(changeset, opts) do
    ...
    if Keyword.get(opts, :validate_unique, true) do
      changeset
      |> unsafe_validate_unique(:email, Sendit.Repo)
  ...
  """
  def valid_password?(%Sendit.Accounts.User{hashed_password: hashed_password}, password)
  ...
  def identity_changeset(user, attrs, _opts \\ []) do
    user
    |> cast(attrs, [:email, :username, :full_name])
    |> validate_required([:email, :username, :full_name])
    |> validate_email_format()
    |> validate_length(:email, max: 160)
